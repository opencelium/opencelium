...
public class BindingUtility {
    ...
    private static Map<String, Object> doWithJsonBody(Map<String, Object> src, List<String> fieldPaths, String id) {
        ...
        for (Map.Entry<String, Object> entry : src.entrySet()) {
            if (name.equals(entry.getKey())) { // object or primitive
                fieldPaths.remove(0);
                resultMap.put(entry.getKey(), process(entry.getValue(), fieldPaths, id, null));
            } else if (name.matches(entry.getKey() + "\\[.*]")) { //array
                fieldPaths.remove(0);
                String index = name.substring(name.indexOf('[') + 1, name.indexOf(']'));
                resultMap.put(entry.getKey(), process(entry.getValue(), fieldPaths, id, index));
            } else {
    ...
    private static Object process(Object value, List<String> fieldPaths, String id, String index) {
        if (index == null) {
