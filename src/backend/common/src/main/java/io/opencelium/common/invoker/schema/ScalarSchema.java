 ...
 */
public record ScalarSchema(ScalarType type, @Nullable String defaultValue) implements Schema {

    public ScalarSchema {
        Objects.requireNonNull(type, "scalar type must not be null");
    }

    /** A copy of this schema with the given default value. */
    public ScalarSchema withDefault(@Nullable String defaultValue) {
        return new ScalarSchema(type, defaultValue);
    }
