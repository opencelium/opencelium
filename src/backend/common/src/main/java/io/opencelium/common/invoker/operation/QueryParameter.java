...
public record QueryParameter(String name, Schema schema, QueryStyle style, boolean explode) {
    ...
    public QueryParameter {
        ...
        Objects.requireNonNull(schema, "schema of query parameter '" + name + "' must not be null");
        Objects.requireNonNull(style, "style of query parameter '" + name + "' must not be null");
        if (name.isBlank()) {
            throw new IllegalArgumentException("query parameter name must not be blank");
        }
        switch (schema) {
            case ScalarSchema scalar -> requireStyle(name, schema, style, QueryStyle.FORM);
            ...
            case ObjectSchema object -> requireStyle(name, schema, style, QueryStyle.FORM, QueryStyle.DEEP_OBJECT);
            case UndefinedSchema undefined -> throw new IllegalArgumentException("query parameter '" + name
