...
public record Pagination(List<PageRule> rules) {
    ...
    /** The rule for a parameter, if this pagination has one. */
    public Optional<PageRule> rule(PageParam param) {
        return rules.stream().filter(rule -> rule.param() == param).findFirst();
    }
}
