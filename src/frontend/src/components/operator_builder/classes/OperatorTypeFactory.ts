    ...
    generateTreeByExpression(expression: string): GroupProps {
        switch (this.type) {
            case OperatorType.If:
                const ifOperatorsConfigGenerator = new IfOperatorsConfigGenerator();
                return ifOperatorsConfigGenerator.generateTreeByExpression(expression);
            case OperatorType.Loop:
                const loopOperatorsConfigGenerator = new LoopOperatorsConfigGenerator();
                return loopOperatorsConfigGenerator.generateTreeByExpression(expression);
        }
    }
