...
class TestConnectionButton extends React.Component {
  ...
  componentDidUpdate(prevProps, prevState, snapshot) {
    ...
    if (
      addingConnection === API_REQUEST_STATE.FINISH &&
      testConnection &&
      startAddingConnection
    ) {
      this.setState({
      ...
      setTimeout(() => {
        addTestSchedule(testConnection);
      }, 300);
    }
    if (
      addingSchedule === API_REQUEST_STATE.FINISH &&
      testSchedule &&
      startAddingSchedule
    ) {
      this.setState({
      ...
      setTimeout(() => {
        startTestSchedule(testSchedule);
      }, 300);
    }
    if (
      startingSchedule === API_REQUEST_STATE.FINISH &&
      testConnection &&
      startTriggeringSchedule &&
      !isFinishedTriggering
    ) {
      this.setState(
        ...
        () => {
          this.scheduleInterval = setInterval(
            () => getScheduleById(testSchedule.schedulerId),
            2000
          );
        }
      );
    ...
    if (
      !isFinishedTriggering &&
      gettingScheduleById === API_REQUEST_STATE.FINISH &&
      currentSchedule &&
      testSchedule &&
      currentSchedule.schedulerId === testSchedule.schedulerId
    ) {
    ...
    if (
      ...
      setTimeout(() => {
        deleteTestScheduleById(testSchedule.schedulerId);
      }, 300);
    }
    if (
      deletingSchedule === API_REQUEST_STATE.FINISH &&
      startDeletingSchedule
    ) {
      this.setState({
        startDeletingSchedule: false,
      ...
      setTimeout(() => {
        deleteTestConnectionById(testConnection.connectionId);
      }, 300);
    }
    if (
      deletingConnection === API_REQUEST_STATE.FINISH &&
      startDeletingConnection
    ) {
      this.setState({
        startDeletingConnection: false,
  ...
  render() {
    ...
    const { isTestingConnection, connection } = this.props;
    const isCreatingConnectionLoading = startAddingConnection;
    const isCreatingScheduleLoading =
      isCreatingConnectionLoading || startAddingSchedule;
    const isExecutionLoading =
      isCreatingScheduleLoading || !isFinishedTriggering;
    const isCleaningLoading =
      isExecutionLoading || startDeletingConnection || startDeletingSchedule;
    return (
      <React.Fragment>
        ...
        >
          <PopoverHeader>{"Test Connection"}</PopoverHeader>
          <PopoverBody>
            <div
              style={{
            ...
            >
              <div>
                <Text
                  value={`Creating a test connection `}
                  size={TextSize.Size_14}
                />
              </div>
              <FontIcon
                isLoading={isCreatingConnectionLoading}
                value={isCreatingConnectionLoading ? " " : "done"}
                size={20}
              />
              <div>
                <Text
                  value={`Creating a test schedule `}
                  size={TextSize.Size_14}
                />
              </div>
              <FontIcon
                isLoading={isCreatingScheduleLoading}
                value={isCreatingScheduleLoading ? " " : "done"}
                size={20}
              />
              <div>
                <Text
                  value={
                    <div>
                      ...
                      <Counter
                        shouldStart={
                          startTriggeringSchedule || startGettingSchedule
                        }
                        shouldStop={!isExecutionLoading}
                        size={TextSize.Size_12}
                      />
                      {")"}
                    </div>
              ...
              <FontIcon
                ...
                value={
                  isExecutionLoading ? " " : isTriggerFailed ? "close" : "done"
                }
                size={20}
              />
              <div>
