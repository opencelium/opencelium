...
@connect(
    mapStateToProps,
    {
        ...
        toggleRequestBodyDialog,
        toggleResponseSuccessBodyDialog,
        toggleResponseFailBodyDialog
    },
    null,
    {forwardRef: true}
)
class TechnicalProcessDescription extends React.Component {
    constructor(props) {
        ...
        this.handleToggleResponseVisibleIcon = this.toggleResponseVisibleIcon.bind(this);
        this.handleUpdateConnection = this.updateConnection.bind(this);
    }

    shouldComponentUpdate(nextProps, nextState) {
        return (
            nextState.isResponseVisible !== this.state.isResponseVisible ||
            ...
            nextProps.isResponseFailDialogOpened !== this.props.isResponseFailDialogOpened ||
            nextProps.isResponseSuccessDialogOpened !== this.props.isResponseSuccessDialogOpened ||
            nextProps.isRequestBodyDialogOpened !== this.props.isRequestBodyDialogOpened ||
            nextProps.toggleRequestBodyDialog !== this.props.toggleRequestBodyDialog ||
            nextProps.toggleResponseSuccessBodyDialog !== this.props.toggleResponseSuccessBodyDialog ||
            nextProps.toggleResponseFailBodyDialog !== this.props.toggleResponseFailBodyDialog ||
    ...
    toggleResponseVisibleIcon() {
        this.setState((prevState) => ({
            isResponseVisible: !prevState.isResponseVisible,
        }));
    }
    ...
    getRenderContext() {
        const { details, connection, theme } = this.props;
        ...
        let invokerName = methodItem && methodItem.invoker && isString(methodItem.invoker.name)
            ? methodItem.invoker.name
            : '';

        let requestFormat = details && isString(methodItem.bodyFormat)
            ? methodItem.bodyFormat
            : '';
        ...
        return {
            ...
            errors,
            hasErrors,
            isErrorLocationRequest,
            isErrorLocationBody,
            errorColor,
        };
    }

    renderGeneralDataEntries(generalDataEntries) {
        return generalDataEntries.map((entry) => (
    ...
    renderRequestSection(context) {
        const {
            readOnly,
            isExtended,
            currentInfo,
            setCurrentInfo,
            isRequestBodyDialogOpened,
            toggleRequestBodyDialog,
            theme,
            connection,
        } = this.props;
        ...
        return (
            <Col xs={12} className={styles.col} style={{marginBottom: '10px'}}>
                <Row className={styles.row}>
                    <Col xs={12} className={styles.col} style={{marginTop: "20px"}}>
                        <b>{`Request`}</b>
                    </Col>
    ...
    renderResponseSection(context) {
        const {
            readOnly,
            isExtended,
            currentInfo,
            setCurrentInfo,
            isResponseFailDialogOpened,
            isResponseSuccessDialogOpened,
            toggleResponseSuccessBodyDialog,
            toggleResponseFailBodyDialog,
            connection,
        } = this.props;
        ...
        return (
            <Col xs={12} className={styles.col}>
                <Row className={styles.row}>
                    ...
                    <Body
                        hasEnhancement={false}
                        toggleBodyDialog={toggleResponseSuccessBodyDialog}
                    ...
                    <Col style={{marginTop: '10px'}} xs={12} className={`${styles.col} ${styles.entry_padding}`}>
                        <b>{`Fail`}</b>
                    </Col>
                    ...
                    <Body
                        hasEnhancement={false}
                        toggleBodyDialog={toggleResponseFailBodyDialog}
                        isBodyDialogOpened={isResponseFailDialogOpened}
    ...
    renderErrorMessages(context) {
        const { isErrorLocationRequest, errors, errorColor } = context;

        if (!isErrorLocationRequest) {
            return null;
        }

        return errors.messages.map((error, index) => (
            <div key={`${index}_${error}`} style={{color: errorColor, width: '100%'}}>
                {error}
            </div>
        ));
    }
    ...
    render() {
        ...
        return (
            <Row className={styles.row} style={{fontSize: `${DefaultTextSize}px`}}>
                ...
                {this.renderGeneralDataEntries(context.generalDataEntries)}
                ...
                {this.renderRequestSection(context)}

                {this.renderErrorMessages(context)}
                ...
                <Col xs={12} className={styles.col} style={{display: 'flex'}} id="response_label">
                    ...
                    <TooltipFontIcon
                        ...
                        onClick={this.handleToggleResponseVisibleIcon}
                        tooltip={isResponseVisible ? 'Hide' : 'Show'}
                        value={isResponseVisible ? 'arrow_drop_up' : 'arrow_drop_down'}
                    />
                </Col>

                {isResponseVisible && this.renderResponseSection(context)}
            </Row>
        );
    }
}
