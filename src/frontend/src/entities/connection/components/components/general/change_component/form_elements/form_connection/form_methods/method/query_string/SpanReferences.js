...
export const LocalReference = (props) => {
    ...
    const displayName = props.fieldName.startsWith('$.') 
    ? props.fieldName.substring(2) 
    : props.fieldName;

    return <span className={`${styles.span_reference_local} ${props.isSelected ? styles.span_reference_selected : ''}`} style={{background: props.color}} data-value="param" data-main={`${props.value}`}>{displayName}</span>;
};