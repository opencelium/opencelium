...
const DataAggregatorDialogForm: FC<IForm> = ({
	...
	const initialScript = CAggregator.splitVariablesFromScript(
		currentAggregator?.script || '',
		isUpdate
	);
	const [variables, setVariables] = useState<string>(
		initialScript.variables || ''
	);
	const [scriptSegment, updateScriptSegment] = useState<string>(
		initialScript.scriptSegment || CAggregator.getScriptSegmentComment()
	);
	...
	useEffect(() => {
		const newMarkers = getMarker(
			scriptSegmentRef.current.editor,
			scriptSegment,
	...
	useEffect(() => {
		if (variablesRef.current) {
			...
			variablesRef.current.editor.session.on('change', () => {
				variablesRef.current.editor.renderer.scrollToLine(
	...
	useEffect(() => {
		if (didMount.current) {
			if (
				error === null &&
				(addingAggregator === API_REQUEST_STATE.FINISH ||
					updatingAggregator === API_REQUEST_STATE.FINISH)
			) {
	...
	useEffect(() => {
		scriptSegmentRef.current.editor.completers.push(
			getStaticWordCompleter(args.map((a) => a.name))
		);
	}, [args]);
	const change = () => {
		if (name === '') {
		...
		if (isAdd) {
			dispatch(
				addAggregator({
					name,
					args,
					script: CAggregator.joinVariablesWithScriptSegment(
						variables,
						scriptSegment
					),
				})
	...
	const updateArgument = (index: number, arg: ModelArgument) => {
		...
		newScriptSegment = replaceVariables(newScriptSegment, {
			[args[index].name]: arg.name,
		});
	...
	const deleteArgument = (index: number) => {
		...
		let newScriptSegment = scriptSegment;
		newScriptSegment = replaceVariables(newScriptSegment, {
			[args[index].name]: CAggregator.generateNotExistVar(),
		});
		setScriptSegment(newScriptSegment);
		const newArgs = [...args];
	...
	if (isAdd || isUpdate) {
		actions.unshift(
			<Button
				...
				handleClick={change}
				isLoading={
					addingAggregator === API_REQUEST_STATE.START ||
					updatingAggregator === API_REQUEST_STATE.START
				}
			/>
		);
	}
	...
	return (
		<FormComponent
			{...data}
			isLoading={
				shouldFetchDataAggregator &&
				(gettingAggregator === API_REQUEST_STATE.START ||
					gettingAllAggregators === API_REQUEST_STATE.START)
			}
		/>
	);
};
