...
export const scheduleSlice = createSlice({
    ...
    extraReducers: {
        ...
        [enableSchedules.fulfilled.type]: (state, action: PayloadAction<SchedulesIdRequestProps>) => {
            ...
            state.schedules = state.schedules.map(schedule => action.payload.identifiers.findIndex(id => id === schedule.schedulerId) !== -1 ? {...schedule, status: 1} : schedule);
            if(state.currentSchedule && action.payload.identifiers.findIndex(id => id === state.currentSchedule.schedulerId) !== -1){
        ...
        [disableSchedules.fulfilled.type]: (state, action: PayloadAction<SchedulesIdRequestProps>) => {
            ...
            state.schedules = state.schedules.map(schedule => action.payload.identifiers.findIndex(id => id === schedule.schedulerId) !== -1 ? {...schedule, status: 0} : schedule);
            if(state.currentSchedule && action.payload.identifiers.findIndex(id => id === state.currentSchedule.schedulerId) !== -1){
