	...
	return (
		...
		>
			<SidebarSearch
				...
				testId='workflow-sidebar-search-secondary'
				autoFocus={!!props.mode}
			/>
			{props.mode === 'connector' ? (
				props.connectorsFetching ? (
				...
				) : props.connectorItems.length ? (
					<SidebarList items={props.connectorItems} testIdPrefix='workflow-sidebar-connector' onSelect={props.onSelectConnector} />
				) : (
				...
				<TriggerConnectionPanel
					isFetching={props.triggerFetching}
					isError={props.triggerError}
					items={props.triggerItems}
					onSelect={props.onSelectTrigger}
				/>
			) : (
				<SidebarList items={props.operatorItems} testIdPrefix='workflow-sidebar-operator' onSelect={props.onSelectOperator} />
			)}
		</SidebarDrawer>
	);
}
