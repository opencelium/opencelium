...
import type { TriggerConnectionScheduleDialogProps } from './TriggerConnectionScheduleDialog.types';

export function TriggerConnectionScheduleDialog({ open, connectionTitle, schedules, onCancel, onConfirm }: TriggerConnectionScheduleDialogProps) {
	const { t } = useI18n('workflow');
	const [schedulerId, setSchedulerId] = useState<number | undefined>();
	...
	return (
		<Dialog
			...
			footer={<>
				<Button onClick={onCancel}>{t('sidebar.triggerConnectionStep.selectScheduleDialog.cancel')}</Button>
				<Button type='primary' disabled={!selectedSchedule} onClick={() => selectedSchedule && onConfirm(selectedSchedule)}>
