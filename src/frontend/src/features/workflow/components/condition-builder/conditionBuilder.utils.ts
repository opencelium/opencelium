...
import { generateTreeByExpression as parseConditionExpression } from './conditionExpressionParser';
...
export const generateTreeByExpression = parseConditionExpression;
...
export const conditionTreeToExpression = (
	...
	const expression = items
		...
		.filter(Boolean)
		.join(` ${conjunction || ''} `);
	if (operatorType !== 'if') return expression;
	const grouped = `(${expression})`;
	return child.properties?.not ? `!${grouped}` : grouped;
};
