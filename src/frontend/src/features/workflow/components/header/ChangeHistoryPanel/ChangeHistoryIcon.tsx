 ...
 */
export function ChangeHistoryIcon({ icon, size = 14 }: Props) {
	...
	if (!icon) {
		return <span className='changeHistoryIcon changeHistoryIcon--empty' aria-hidden />;
	}
	...
	if (icon.kind === 'connector') {
		...
		return resolveConnectorIconUrl(icon.iconUrl)
			? (
				<span className='changeHistoryIcon changeHistoryIcon--logo'>
					<ConnectorIcon icon={icon.iconUrl} size={size} style={{ flexShrink: 0 }} />
				</span>
			)
			: <span className='changeHistoryIcon'><Icon name='connector' size={size} /></span>;
	}

	return <span className='changeHistoryIcon'><Icon name={icon.kind} size={size} /></span>;
}
