        ...
        <div>
            ...
            {options.map(option => {
                ...
                return (
                    ...
                    >
                        <span className="onboarding-theme-card__content">
    ...
    return (
        <div>
            ...
            <div className="onboarding-copy-grid">
                <div><strong>{t('content.palette.tokens')}</strong><span>{t('content.palette.tokensBody')}</span><code>connector create</code><code>invoker upload</code><code>schedule run</code></div>
                <div><strong>{t('content.palette.help')}</strong><span>{t('content.palette.helpBody')}</span><code className="onboarding-help-token">help</code></div>
            </div>
        </div>
    )
}
...
export function FirstInvokerContent({ onCreateManually, onUploaded, onGitDownloaded }: { onCreateManually: () => void; onUploaded: () => void; onGitDownloaded: () => void }) {
    const { t } = useI18n('onboarding')
        ...
        <div>
            ...
            <div className="onboarding-invoker-options">
                <section className="is-recommended">
                    <div><h3>{t('content.invoker.uploadTitle')} <span className="onboarding-recommended">{t('content.invoker.recommended')}</span></h3><p>{t('content.invoker.uploadBody')}</p></div>
                    <div
                    ...
                    >
                        {uploadState === 'success' ? (
                            <><strong>✓ {t('content.invoker.fileAdded', { name: fileName })}</strong><small>{[t('content.invoker.methods', { count: uploadMetadata?.methodCount ?? 0 }), uploadMetadata?.authType, uploadMetadata?.version].filter(Boolean).join(' · ')}</small><small>{t('content.invoker.ready')}</small></>
                        ) : uploadState === 'error' ? (
                            <><strong>× {t('content.invoker.fileError', { name: fileName })}</strong><small>{t('content.invoker.fileFormat')}</small><b>{t('content.invoker.chooseAnother')}</b></>
                        ) : isUploading ? (
                            <><strong>{fileName}</strong><span className="onboarding-upload-progress"><i /></span><small>{t('content.invoker.parsing')}</small></>
                        ) : (
                            <><Upload size={18} strokeWidth={1.8} aria-hidden /><span>{isDragging ? t('content.invoker.release') : t('content.invoker.drop')}</span></>
                        )}
                        <InvokerUploadButton
                            ref={uploadRef}
                ...
                <section>
                    <div><h3>{t('content.invoker.gitTitle')}</h3><p>{t('content.invoker.gitBody')}</p><code>github.com/opencelium/invokers · branch main</code></div>
                    <Button type="default" loading={gitLoading} onClick={() => { setGitLoading(true); window.setTimeout(() => { setGitLoading(false); onGitDownloaded() }, 700) }}>{t('content.invoker.gitAction')}</Button>
                </section>
                <section>
